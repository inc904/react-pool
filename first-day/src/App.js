...
const TimeCard = ({ time }) => {
  ...
  // const localeTime = formatTime(time);
   const localeTime = useMemo(() => formatTime(time), [time])
  return (
    <div className="time-card">
      <div className="time-card-header">
        <div className="time-card-title">最新:{localeTime}</div>
      </div>
    </div>
  );
};
const App = () => {
  ...
  return (
    <div className="app">
      ...
      <TimeCard time={time} />
      <button variant="outlined" onClick={() => setTime(Date.now())}>
        更新时间
      </button>
      <button
        variant="outlined"
